...
def update_clusters():
    ...
    if num_reviews % update_step == 0:
        ...
        all_whisky_ids = set(map(lambda x: x.whisky.id, Review.objects.only("whisky")))
        num_users = len(all_user_names)
        ratings_m = dok_matrix((num_users, max(all_whisky_ids)+1), dtype=np.float32)
        for i in range(num_users): # Each user corresponds to a row, in the order of all_user_names
            ...
            for user_review in user_reviews:
                ratings_m[i, user_review.whisky_id] = user_review.rating
